...
const useChatStore = create((set, get) => ({
    ...
    createOneToOneChat: async (userId) => {
        try {
            const { data } = await axiosInstance.post('/chat/oneToOneChat', { userId });
            set((state) => ({ chats: [data, ...state.chats] }));
            toast.success('Chat created successfully!');
            return data;
        } catch (error) {
            console.error('Error creating one-to-one chat:', error.response?.data || error.message);
        }
    },
    ...
    renameGroup: async (chatId, newName) => {
        try {
            const { data } = await axios.put('/api/chat/rename', { chatId, chatName: newName });
            set((state) => ({
                chats: state.chats.map((chat) => (chat._id === chatId ? data : chat)),
            }));
            return data;
        } catch (error) {
            console.error('Error renaming group:', error.response?.data || error.message);
        }
    },
    ...
    updateGroupIcon: async (chatId, file) => {
        try {
            ...
            console.error('Error updating group icon:', error.response?.data || error.message);
        }
    },

    deleteGroupIcon: async (chatId) => {
        try {
            const { data } = await axios.put('/api/chat/delete-group-icon', { chatId });
            set((state) => ({
                chats: state.chats.map((chat) => (chat._id === chatId ? data : chat)),
            }));
            return data;
        } catch (error) {
            console.error('Error deleting group icon:', error.response?.data || error.message);
        }
    },
